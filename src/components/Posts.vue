<template>
  <div>
    <el-row type="flex" justify="center" class="post-content">
      <el-col :span="16" :xs="{span: 24}" :sm="{span: 23}" :md="{span: 23}" :lg="{span: 16}" class="post-card" id="post-card">
        ...
        <div v-else>
          ...
          <div class="btn-group">
            <el-button type="primary" v-if="tab === 'Write' || type === 'add'" @click="handleCancelEdit">取消</el-button>
            <el-button type="primary" @click="handleSubmit">确定</el-button>
          </div>
        </div>
        <span id="footerPost"></span>
      </el-col>
      <el-col :span="6" class="post-toc" id="post-toc" v-if="type === 'view'" :class="{'open-toc': hasToc}">
        <h4 class="catalog-title">TOC</h4>
        <div class="catalog-body">
          <ul id="catalog-list" class="catalog-list">
            <li v-for="(item,index) in catalogList" :key="index" class="toc-li" :class="{active:currentIndex === index}">
              <a
    ...
    <el-row type="flex" justify="center" v-if="type === 'view'">
      <el-col :span="23" v-if="$themeConfig.vssue.need">
        <my-vssue/>
      </el-col>
    </el-row>
...
export default {
	...
	components: {
		TocBtn,
		MyVssue: () => import("@/components/MyVssue")
	},
	data() {
		return {
			tab: "Write",
      ...
      viewMdText: '',
			...
			compiledMd: "",
			id: undefined,
			type: "" // add 添加文章 edit 编辑文章 view 查看文章
		}
	},
	props: {
		minHeight: Number
    ...
    ...mapActions(['loadPosts']),
        ...
        this.setPost({title: post.title, date: post.lastUpdated})
			} else if (params.post === "add") {
				this.type = "add"
        this.id = undefined
        this.tab = 'Write'
        this.mdText = defaultMD
        this.setPost({ title: '新增' })
        this.$refs.textarea && this.$refs.textarea.focus()
			} else {
				this.type = "view"
        this.id = params.post
